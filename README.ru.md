...
**Image-генерация** (тот же промпт, разные рендер-модели — балл 0–10, S5 — самый сложный слот):
...
Разрыв проходит по S5 (environmental departure): с общим баллом 7+ модель всё ещё может валиться на самом сложном слоте — поэтому Flux-2-max и Imagen 4.0 допущены только на начальные слоты, хотя средний балл у них неплохой.
