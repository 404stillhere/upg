# UNIVERSAL PROMPT GENERATOR v6.1
...
generation, and validation checks before the prompts.
...
The register established here governs all slot prompts (enforced in Phase 5.2).
...
| **2** | TILT (one element shifted) | Enriched Theme + Entry Angle at different scale/setting | Instant | Differs from Slot 1 in: scale OR setting OR mood |
| **3** | COLLISION (theme + creative lens) | SEED 1 | <5 sec | Differs from Slot 2 in: scale OR setting OR mood |
| **4** | DRIFT (lens dominates, theme undertone) | SEED 2 | <15 sec | Differs from Slot 3 in: scale OR setting OR mood |
| **5** | RUPTURE (maximum departure, theme felt not seen) | SEED 3 | <30 sec | Differs from Slot 4 in: scale OR setting OR mood |
...
1. **Opening:** The dominant visual idea — the most important element the viewer
   sees first
2. **Subject with material specificity:** What it is made of, surface detail,
   tactile quality
3. **Environment and spatial context:** Where, depth, surroundings, scale
4. **Light:** Source, direction, quality, color, and what it DOES
   (casts, filters, catches, pools, rakes)
5. **Palette:** 3–5 specific color names
   ("burnt sienna, raw umber, pale gold" — not "warm tones")
6. **Emotional quality:** Expressed through visual detail, not adjectives
...
*End of UNIVERSAL PROMPT GENERATOR v6.0*
