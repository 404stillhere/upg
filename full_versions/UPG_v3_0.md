# UNIVERSAL PROMPT GENERATOR v3.0
...
**Purpose:** Generate subtle, non-obvious associative elements that enrich visual depth without forcing meaning. These artifacts bridge ENRICHED THEME → visual storytelling at granular level.

**Algorithm:** Build associative chains using cultural reference → lateral connection → unexpected domain. Avoid obvious symbols (Eiffel Tower for Paris), favor sophisticated paths (Paris → café → chess → checkerboard floor → optical illusion).

**3.5a. Primary associations:**

Generate 2–3 direct cultural, historical, or linguistic links to ENRICHED THEME:
...
*Example: THEME "detective story" → "deduction" / "observation" / "solitude"*

**3.5b. Secondary associations:**

Chain each primary association through 1–2 lateral hops into unexpected domains (3–4 total):
...
*Example: "deduction" → "logic" → "ancient rhetoric" → "marble busts" or "library scrolls"*

**3.5c. Symbolic artifacts:**

Distill the most refined secondary associations into 2–3 final visual artifacts — concrete, visually encodable elements:

- `[artifact 1]: [final element + visual descriptor]`
- `[artifact 2]: [final element + visual descriptor]`
- `[artifact 3]: [final element + visual descriptor]`

*Example: "marble bust with chipped nose", "unrolled parchment with underlined passages", "worn leather journal with ink stains"*

**3.5d. Classification:**

Categorize selected artifacts by integration method:

- **Situational:** Artifacts that modify scene context or setting (furniture, architectural detail, atmospheric element)
- **Contextual:** Artifacts that add environmental richness without narrative weight (worn texture, scattered objects, aged patina)
- **Symbolic:** Artifacts that carry metaphorical weight and deepen meaning (objects tied to emotional arc, visual echoes of theme)
...
Apply ruthless quality check — remove anything recognizable at first glance, retain only sophisticated choices:

- `Rejected (too obvious): [list — these are banalities]`
- `Selected (appropriately subtle): [list — these earn a place in visual]`

**Artifact distribution across spectrum:**

- **Slots 1–2:** Contextual artifacts (reinforce grounding, add detail without disruption)
- **Slot 3:** Situational artifacts (anchor the collision zone with environmental specificity)
- **Slots 4–5:** Symbolic artifacts (resonate with inversion / rupture, carry thematic weight into unexpected territory)

Declare: `ARTIFACT DISTRIBUTION: Slot 1-2 [artifacts], Slot 3 [artifacts], Slot 4-5 [artifacts]`

---

## STEP 4 — INTERNAL REASONING (Chain of Thought)

**This step must be written out explicitly inside a `<scratchpad>` block before any final output is produced. Do not skip or summarize — fully execute each sub-step in writing.**

```xml
<scratchpad>

ARCHETYPE CANDIDATES (generate 6, then select the 4 strongest):
1. [archetype name] — [one-line visual concept]
2. [archetype name] — [one-line visual concept]
3. [archetype name] — [one-line visual concept]
4. [archetype name] — [one-line visual concept]
5. [archetype name] — [one-line visual concept]
6. [archetype name] — [one-line visual concept]

SELECTED ARCHETYPES: [list the 4 strongest and briefly state why each was chosen over the rejected two]

ARTIFACT INTEGRATION:

* Slot 1: [which artifacts from 3.5 + method of integration — situational or contextual]
* Slot 2: [which artifacts from 3.5 + method of integration — situational or contextual]
* Slot 3: [which artifacts from 3.5 + method of integration — situational or collision-heightening]
* Slot 4: [which artifacts from 3.5 + method of integration — symbolic, drift-supporting]
* Slot 5: [which artifacts from 3.5 + method of integration — symbolic, rupture-deepening]

SCENE MATRIX (4 archetypes × 3 variations = 12 scenes):
Archetype 1:
  - Variation A: [scene sketch]
  - Variation B: [scene sketch]
  - Variation C: [scene sketch]
Archetype 2:
  - Variation A: [scene sketch]
  - Variation B: [scene sketch]
  - Variation C: [scene sketch]
...
TOURNAMENT — Select top 5 scenes, one per spectrum slot:
- Slot 1 (Literal 100/0): [chosen scene + reason]
- Slot 2 (Leaning 80/20): [chosen scene + reason]
- Slot 3 (Collision 50/50): [chosen scene + reason]
- Slot 4 (Drift 20/80): [chosen scene + reason]
- Slot 5 (Rupture 0/100): [chosen scene + reason]

COHERENCE PASS:
- Does every slot contain the SUBJECT ANCHOR? [YES / NO — fix if NO]
- Does every slot respect TOPIC LOCK? [YES / NO — fix if NO]
- Does every slot respect GOAL LOCK? [YES / NO — fix if NO]
- Does every slot respect FORMAT LOCK? [YES / NO — fix if NO]

</scratchpad>
```

Only after completing the scratchpad, proceed to Step 5.
...
**Prompt density guidance** (use semantic length targets — do not count words):
- Slot 1: Medium descriptive — clear, grounded, well-specified scene with no flourishes
- Slot 2: Medium descriptive — introduce one unexpected element without disrupting clarity
- Slot 3: Rich, multi-clause — balanced tension between literal and unexpected requires more layering
- Slot 4: Rich, multi-clause — dominant unexpected logic needs full atmospheric build-out
- Slot 5: Short and punchy OR rich and surreal — rupture works either as a single striking image or a dense hallucinatory scene; choose based on Rupture Type

Use Appendix C for platform-specific density calibration. Remove elements bottom-up:
...
```prompt
[Slot 1 prompt here — medium descriptive, fully grounded]
```
...
```prompt
[Slot 2 prompt here — medium descriptive, one unexpected touch]
```
...
```prompt
[Slot 3 prompt here — rich, multi-clause, balanced tension]
```
...
```prompt
[Slot 4 prompt here — rich, multi-clause, dominant unexpected logic]
```
...
```prompt
[Slot 5 prompt here — short and punchy OR dense and surreal, per Rupture Type]
```
...
> ⚠️ **Platform rule:** For modern natural-language engines — Flux, xAI Aurora, Midjourney v6+, DALL-E 3, Firefly, Gemini Imagen — **omit negative prompts entirely** unless a specific structural constraint makes them necessary (e.g., "no text overlay", "no people"). These engines interpret positive descriptions; negatives often degrade quality or are ignored.
>
> **For Stable Diffusion / SDXL only** — include the appropriate negative prompt block from Appendix D.
>
> If SD is your target platform, paste the relevant block below:

```prompt
[SD/SDXL negative prompt — from Appendix D, if applicable]
```
...
## Appendix C — Platform Density Calibration & Reduction Order

Rather than targeting specific word counts (which LLMs cannot reliably control), calibrate prompt density to platform expectations:

| Platform | Density target |
|---|---|
| Midjourney | Compact — essential scene elements only, no long subordinate clauses |
| Flux / Aurora / Gemini Imagen | Full descriptive — rich layering of scene, light, texture, and mood encouraged |
| SD / SDXL | Moderately detailed — keyword-friendly phrasing, avoid overly complex syntax |
| Firefly | Moderate — natural language preferred, mid-level detail |

**Reduction order** (remove from bottom first when compressing for a shorter platform):
...
## Appendix D — Negative Prompts by Medium (SD / SDXL only)

> Use these only when targeting **Stable Diffusion or SDXL**. Omit for all other platforms unless a specific structural override is needed.
...
*Universal Prompt Generator v3.0 — Photography, painting, illustration, concept art, pixel art, avatar.*
